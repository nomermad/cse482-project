...
def train_models(df):
    ...
    df_sampled['userId'] = df_sampled['userId'].astype(str)
    df_sampled['movieId'] = df_sampled['movieId'].astype(str)
    ...
    X = pd.concat([
        pd.DataFrame({
            'userId_encoded': df_sampled['userId_encoded'],
            'movieId_encoded': df_sampled['movieId_encoded']
        }),
        X
    ], axis=1)
